...
STYLE = """
<style>
:root{--bg:#eef3f9;--card:#fff;--ink:#10203d;--muted:#71809a;--blue:#2563eb;--nav:#071a38;--ok:#13a36f;--bad:#ef4444;--purple:#7048e8;--orange:#f59e0b;--personal:#111c3d}
...
a{color:inherit}.topbar{height:58px;background:#06152f;color:#fff;display:flex;align-items:center;justify-content:space-between;padding:0 18px;position:sticky;top:0;z-index:20}.brand{font-size:23px;font-weight:900}.version{font-size:11px;background:#22c55e22;color:#4ade80;padding:4px 8px;border-radius:20px}.userbox{font-size:13px}
.shell{display:grid;grid-template-columns:190px minmax(0,3fr) minmax(290px,1fr);min-height:calc(100vh - 58px)}.sidebar{background:linear-gradient(180deg,#09234a,#06152f);color:#fff;padding:14px 10px}.menu-title{font-size:12px;color:#c7d2e5;padding:8px 10px}.nav{display:grid;gap:5px}.nav a{color:#fff;text-decoration:none;padding:10px 11px;border-radius:10px;font-size:13px}.nav a:hover{background:#ffffff17}.nav a:first-child{background:linear-gradient(135deg,#1d64ee,#3478ff)}
.company{padding:16px;min-width:0}.personal{background:linear-gradient(180deg,#171b46,#07152f);color:#fff;padding:14px;min-width:0}.personal .card{background:#ffffff0e;color:#fff;border:1px solid #ffffff0d;box-shadow:none}.personal .muted{color:#b8c2da}.personal h2{color:#fff}.personal table{color:#fff}.personal th{color:#b8c2da}.personal td,.personal th{border-color:#ffffff12}
.wrap{max-width:none;margin:0;padding:0}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.card{background:#fff;border-radius:16px;padding:16px;box-shadow:0 6px 22px #1d2d4d12}.kpi{border-top:4px solid var(--blue)}.kpi.green{border-color:var(--ok)}.kpi.red{border-color:var(--bad)}.kpi.purple{border-color:var(--purple)}.kpi.orange{border-color:var(--orange)}.big{font-size:25px;font-weight:900;margin-top:8px}.muted{color:var(--muted);font-size:12px}.section{margin-top:14px}h2{margin:0 0 12px;font-size:18px}h3{margin:0 0 8px}
.quick{display:block;text-decoration:none;transition:.15s}.quick:hover{transform:translateY(-2px)}.quick.blue{background:#eaf1ff}.quick.green{background:#e9fbf3}.quick.purple{background:#f1ecff}.quick.orange{background:#fff3e4}
...
table{width:100%;border-collapse:collapse;font-size:13px}th,td{padding:10px;border-bottom:1px solid #e6ebf2;text-align:right}th{color:var(--muted);font-size:11px}.badge{padding:5px 8px;border-radius:20px;font-size:11px;background:#e8eef8}.badge.approved,.badge.paid,.badge.good{background:#dcf8eb;color:#08734c}.badge.pending,.badge.normal{background:#fff1d6;color:#996000}.badge.rejected,.badge.bad,.badge.overdue{background:#ffe1e5;color:#a51d31}.flash{background:#fff3cd;padding:11px;border-radius:10px;margin-bottom:12px}.login{max-width:430px;margin:70px auto;padding:16px}.ltr{direction:ltr;text-align:left}.personal-actions{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.personal-actions .btn{font-size:11px;text-align:center;padding:10px 5px}.personal-summary{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.personal-summary .card{padding:12px}.expense-bar{height:10px;border-radius:10px;background:#ffffff18;overflow:hidden}.expense-bar span{display:block;height:100%;background:linear-gradient(90deg,#6d5dfc,#ee4d8a)}
@media(max-width:1150px){.shell{grid-template-columns:170px minmax(0,3fr) minmax(250px,1fr)}.grid{grid-template-columns:repeat(2,1fr)}}
@media(max-width:850px){.shell{grid-template-columns:1fr}.sidebar{display:none}.personal{order:3}.company{padding:11px}.grid,.grid3{grid-template-columns:1fr 1fr}.row,.row3{grid-template-columns:1fr}table{font-size:11px}}
@media(max-width:520px){.grid,.grid3{grid-template-columns:1fr}.personal-summary{grid-template-columns:1fr 1fr}.big{font-size:21px}}
</style>"""

BASE = """<!doctype html><html lang="fa" dir="rtl"><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="utf-8"><title>{{title}} | BuildAI ERP</title>""" + STYLE + """</head><body>
{% if session.get('user_id') %}<div class="topbar"><div class="brand">BuildAI ERP <span class="version">v{{version}}</span></div><div class="userbox">{{session.get('full_name')}} / Owner</div></div><div class="shell"><aside class="sidebar"><div class="menu-title">منوی اصلی شرکت / Company Menu</div><div class="nav">
<a href="{{url_for('dashboard')}}">▣ داشبورد / Dashboard</a><a href="{{url_for('projects')}}">▥ پروژه‌ها / Projects</a><a href="{{url_for('customers')}}">♙ مشتریان / Customers</a><a href="{{url_for('contracts')}}">▤ قراردادها / Contracts</a><a href="{{url_for('invoices')}}">▧ فاکتورها / Invoices</a><a href="{{url_for('suppliers')}}">♧ تأمین‌کنندگان / Suppliers</a><a href="{{url_for('inventory')}}">⌂ انبار مصالح / Inventory</a><a href="{{url_for('employees')}}">♙ کارکنان / Employees</a><a href="{{url_for('accounts')}}">▣ صندوق و بانک / Cash & Bank</a><a href="{{url_for('financial_report')}}">⌁ گزارش‌ها / Reports</a><a href="{{url_for('new_expense')}}">− هزینه جدید / Expense</a><a href="{{url_for('new_income')}}">＋ درآمد جدید / Income</a>{% if session.get('role') in ['admin','manager','accountant'] %}<a href="{{url_for('approvals')}}">✓ تأیید هزینه‌ها</a>{% endif %}{% if session.get('role') == 'admin' %}<a href="{{url_for('users')}}">⚙ کاربران</a><a href="{{url_for('activity')}}">◷ فعالیت‌ها</a>{% endif %}<a href="{{url_for('logout')}}">↪ خروج / Logout</a></div></aside><main class="company"><div class="wrap">{% with messages=get_flashed_messages() %}{% for m in messages %}<div class="flash">{{m}}</div>{% endfor %}{% endwith %}{{body|safe}}</div></main><aside class="personal">{{personal|safe}}</aside></div>{% else %}<div class="wrap">{{body|safe}}</div>{% endif %}</body></html>"""


def personal_panel():
    if not session.get("user_id"): return ""
    c=get_db(); uid=session["user_id"]
    inc=c.execute("SELECT COALESCE(SUM(amount),0) s FROM personal_transactions WHERE user_id=? AND transaction_type='income'",(uid,)).fetchone()["s"]
    exp=c.execute("SELECT COALESCE(SUM(amount),0) s FROM personal_transactions WHERE user_id=? AND transaction_type='expense'",(uid,)).fetchone()["s"]
    cats=c.execute("SELECT category,SUM(amount) total FROM personal_transactions WHERE user_id=? AND transaction_type='expense' GROUP BY category ORDER BY total DESC LIMIT 6",(uid,)).fetchall()
    rows=c.execute("SELECT * FROM personal_transactions WHERE user_id=? ORDER BY transaction_date DESC,id DESC LIMIT 6",(uid,)).fetchall(); c.close()
    return render_template_string("""<h2>حساب شخصی / Personal Account</h2><div class='personal-summary'><div class='card'><div class='muted'>درآمد شخصی</div><div class='big'>{{inc|money}}</div></div><div class='card'><div class='muted'>هزینه شخصی</div><div class='big'>{{exp|money}}</div></div><div class='card'><div class='muted'>سود خالص</div><div class='big'>{{(inc-exp)|money}}</div></div><div class='card'><div class='muted'>موجودی</div><div class='big'>{{(inc-exp)|money}}</div></div></div><div class='section personal-actions'><a class='btn ok' href='{{url_for("personal_new",kind="income")}}'>＋ درآمد</a><a class='btn bad' href='{{url_for("personal_new",kind="expense")}}'>− هزینه</a><a class='btn' href='{{url_for("personal_transactions")}}'>همه تراکنش‌ها</a></div><div class='card section'><h3>دسته‌های هزینه</h3>{% for x in cats %}<div style='margin:10px 0'><div style='display:flex;justify-content:space-between;font-size:12px'><span>{{x.category}}</span><span>{{x.total|money}}</span></div><div class='expense-bar'><span style='width:{{ [100,(x.total/(exp or 1))*100]|min }}%'></span></div></div>{% else %}<div class='muted'>هنوز هزینه‌ای ثبت نشده است.</div>{% endfor %}</div><div class='card section'><h3>تراکنش‌های اخیر</h3><table>{% for x in rows %}<tr><td>{{x.description or x.category}}</td><td>{{x.amount|money}}</td></tr>{% else %}<tr><td class='muted'>بدون تراکنش</td></tr>{% endfor %}</table></div>""",inc=inc,exp=exp,cats=cats,rows=rows)
...
def dashboard():
    ...
    recent=c.execute("SELECT e.*,u.full_name,p.name project_name FROM expenses e JOIN users u ON u.id=e.user_id LEFT JOIN projects p ON p.id=e.project_id ORDER BY e.id DESC LIMIT 6").fetchall()
    ...
    invs=c.execute("SELECT i.*,c.name customer_name FROM invoices i LEFT JOIN customers c ON c.id=i.customer_id ORDER BY i.id DESC LIMIT 5").fetchall(); c.close()
    return page("داشبورد","""<div class='grid'><div class='card kpi green'><div class='muted'>کل درآمد / Total Income</div><div class='big'>{{income|money}}</div></div><div class='card kpi red'><div class='muted'>کل هزینه / Total Expenses</div><div class='big'>{{expense|money}}</div></div><div class='card kpi purple'><div class='muted'>سود خالص / Net Profit</div><div class='big'>{{(income-expense)|money}}</div></div><div class='card kpi orange'><div class='muted'>مطالبات / Receivable</div><div class='big'>{{receivable|money}}</div></div></div>
    <div class='grid section'><a class='card quick blue' href='{{url_for("projects")}}'><h3>＋ پروژه جدید</h3><div class='muted'>New Project</div></a><a class='card quick green' href='{{url_for("customers")}}'><h3>＋ مشتری جدید</h3><div class='muted'>New Customer</div></a><a class='card quick purple' href='{{url_for("new_invoice")}}'><h3>＋ فاکتور جدید</h3><div class='muted'>New Invoice</div></a><a class='card quick orange' href='{{url_for("new_expense")}}'><h3>＋ هزینه جدید</h3><div class='muted'>New Expense</div></a></div>
    <div class='grid3 section'><div class='card' style='grid-column:span 2'><h2>پروژه‌های فعال / Active Projects</h2><table><tr><th>پروژه</th><th>مشتری</th><th>بودجه</th><th>سود فعلی</th></tr>{% for p in projs %}<tr><td>{{p.name}}</td><td>{{p.customer or '-'}}</td><td>{{p.budget|money}}</td><td>{{(p.income-p.expense)|money}}</td></tr>{% else %}<tr><td colspan='4'>پروژه‌ای ثبت نشده است</td></tr>{% endfor %}</table></div><div class='card'><h2>خلاصه</h2><div class='big'>{{projects_count}}</div><div class='muted'>پروژه فعال</div><hr><div class='big'>{{pending|money}}</div><div class='muted'>هزینه منتظر تأیید</div></div></div>
    <div class='grid3 section'><div class='card' style='grid-column:span 2'><h2>آخرین هزینه‌ها / Recent Expenses</h2><table><tr><th>پروژه</th><th>شرح</th><th>مبلغ</th><th>وضعیت</th></tr>{% for x in recent %}<tr><td>{{x.project_name or '-'}}</td><td>{{x.description}}</td><td>{{x.amount|money}}</td><td><span class='badge {{x.status}}'>{{x.status}}</span></td></tr>{% endfor %}</table></div><div class='card'><h2>فاکتورهای اخیر</h2><table>{% for x in invs %}<tr><td>{{x.invoice_no}}</td><td>{{x.total|money}}</td></tr>{% else %}<tr><td>بدون فاکتور</td></tr>{% endfor %}</table></div></div>""",income=income,expense=expense,pending=pending,receivable=receivable,projects_count=projects_count,recent=recent,projs=projs,invs=invs)
