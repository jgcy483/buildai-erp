...
def dashboard():
    c=get_db()
    income=c.execute("SELECT COALESCE(SUM(amount),0) s FROM incomes").fetchone()["s"]
    expense=c.execute("SELECT COALESCE(SUM(amount),0) s FROM expenses WHERE status='approved'").fetchone()["s"]
    pending=c.execute("SELECT COALESCE(SUM(amount),0) s FROM expenses WHERE status='pending'").fetchone()["s"]
    ...
    recent=c.execute("SELECT e.*,u.full_name,p.name project_name FROM expenses e JOIN users u ON u.id=e.user_id LEFT JOIN projects p ON p.id=e.project_id ORDER BY e.id DESC LIMIT 5").fetchall()
    projs=c.execute("SELECT p.*,COALESCE((SELECT SUM(amount) FROM incomes i WHERE i.project_id=p.id),0) income,COALESCE((SELECT SUM(amount) FROM expenses e WHERE e.project_id=p.id AND e.status='approved'),0) expense FROM projects p ORDER BY p.id DESC LIMIT 4").fetchall()
    invs=c.execute("SELECT i.*,c.name customer_name FROM invoices i LEFT JOIN customers c ON c.id=i.customer_id ORDER BY i.id DESC LIMIT 4").fetchall()
    trans=c.execute("SELECT 'income' kind,i.description,i.amount,i.income_date d,p.name project FROM incomes i LEFT JOIN projects p ON p.id=i.project_id UNION ALL SELECT 'expense',e.description,e.amount,e.expense_date,p.name FROM expenses e LEFT JOIN projects p ON p.id=e.project_id WHERE e.status='approved' ORDER BY d DESC LIMIT 5").fetchall()
    c.close()
    return page("داشبورد","""<div class='grid'>
    <div class='card kpi green'><div class='kpi-icon'>↗</div><div class='muted'>کل درآمد / Total Income</div><div class='big'>{{income|money}}</div></div>
    <div class='card kpi red'><div class='kpi-icon'>↘</div><div class='muted'>کل هزینه / Total Expenses</div><div class='big'>{{expense|money}}</div></div>
    <div class='card kpi purple'><div class='kpi-icon'>▥</div><div class='muted'>سود خالص / Net Profit</div><div class='big'>{{(income-expense)|money}}</div></div>
    <div class='card kpi blue'><div class='kpi-icon'>▣</div><div class='muted'>مطالبات / Receivables</div><div class='big'>{{receivable|money}}</div></div>
    <div class='card kpi orange'><div class='kpi-icon'>▤</div><div class='muted'>بدهی‌ها / Payables</div><div class='big'>{{payables|money}}</div></div></div>
    <div class='dash2 section'><div class='card'><div class='section-head'><h2>پروژه‌های فعال / Active Projects</h2><a href='{{url_for("projects")}}'>مشاهده همه</a></div><table><tr><th>پروژه</th><th>مشتری</th><th>پیشرفت مالی</th><th>سود فعلی</th></tr>{% for p in projs %}{% set progress=((p.expense/(p.budget or 1))*100) %}<tr><td><b>{{p.name}}</b><div class='muted'>{{p.location or ''}}</div></td><td>{{p.customer or '-'}}</td><td><div>{{'%.0f'|format([progress,100]|min)}}%</div><div class='progress'><span style='width:{{[progress,100]|min}}%'></span></div></td><td style='color:#0c9b68;font-weight:900'>{{(p.income-p.expense)|money}}</td></tr>{% else %}<tr><td colspan='4'>پروژه‌ای ثبت نشده است</td></tr>{% endfor %}</table></div>
    <div class='card'><div class='section-head'><h2>نمودار درآمد و هزینه</h2><span class='muted'>ماه جاری</span></div><div class='legend'><span><i class='dot' style='background:#12a56f'></i>درآمد</span><span><i class='dot' style='background:#ef4444'></i>هزینه</span></div><div class='chart'><div class='chart-grid'></div><svg viewBox='0 0 600 200' preserveAspectRatio='none'><polyline fill='none' stroke='#12a56f' stroke-width='5' points='0,160 70,125 130,140 200,85 270,105 335,62 405,88 470,60 535,68 600,35'/><polyline fill='none' stroke='#ef4444' stroke-width='5' points='0,175 70,150 130,160 200,125 270,138 335,112 405,130 470,95 535,82 600,105'/></svg></div></div></div>
    <div class='dash2 section'><div class='card'><div class='section-head'><h2>فاکتورهای اخیر / Recent Invoices</h2><a href='{{url_for("invoices")}}'>مشاهده همه</a></div><table><tr><th>شماره</th><th>مشتری</th><th>مبلغ</th><th>وضعیت</th></tr>{% for x in invs %}<tr><td><b>{{x.invoice_no}}</b></td><td>{{x.customer_name or '-'}}</td><td>{{x.total|money}}</td><td><span class='badge {{x.status}}'>{{x.status}}</span></td></tr>{% else %}<tr><td colspan='4'>بدون فاکتور</td></tr>{% endfor %}</table></div>
    <div class='card'><div class='section-head'><h2>تراکنش‌های اخیر / Recent Transactions</h2><span class='muted'>{{projects_count}} پروژه فعال</span></div><table><tr><th>شرح</th><th>پروژه</th><th>مبلغ</th></tr>{% for x in trans %}<tr><td>{{'↓' if x.kind=='income' else '↑'}} {{x.description}}</td><td>{{x.project or '-'}}</td><td style='font-weight:900;color:{{"#12a56f" if x.kind=="income" else "#ef4444"}}'>{{('+' if x.kind=='income' else '-') ~ (x.amount|money)}}</td></tr>{% else %}<tr><td colspan='3'>بدون تراکنش</td></tr>{% endfor %}</table></div></div>
    <div class='quick-row section'><a class='card quick blue' href='{{url_for("projects")}}'><div class='qicon'>🏗️</div><b>پروژه جدید</b><div class='muted'>New Project</div></a><a class='card quick green' href='{{url_for("customers")}}'><div class='qicon'>👥</div><b>مشتری جدید</b><div class='muted'>New Customer</div></a><a class='card quick purple' href='{{url_for("new_contract")}}'><div class='qicon'>📑</div><b>قرارداد جدید</b><div class='muted'>New Contract</div></a><a class='card quick blue' href='{{url_for("new_invoice")}}'><div class='qicon'>🧾</div><b>فاکتور جدید</b><div class='muted'>New Invoice</div></a><a class='card quick red' href='{{url_for("new_expense")}}'><div class='qicon'>💳</div><b>هزینه جدید</b><div class='muted'>New Expense</div></a><a class='card quick green' href='{{url_for("new_income")}}'><div class='qicon'>💰</div><b>درآمد جدید</b><div class='muted'>New Income</div></a><a class='card quick orange' href='{{url_for("inventory")}}'><div class='qicon'>📦</div><b>انبار</b><div class='muted'>Inventory</div></a><a class='card quick purple' href='{{url_for("financial_report")}}'><div class='qicon'>📊</div><b>گزارش‌ها</b><div class='muted'>Reports</div></a><a class='card quick teal' href='{{url_for("accounts")}}'><div class='qicon'>🏦</div><b>صندوق و بانک</b><div class='muted'>Cash & Bank</div></a></div>""",income=income,expense=expense,pending=pending,receivable=receivable,payables=payables,projects_count=projects_count,recent=recent,projs=projs,invs=invs,trans=trans)
