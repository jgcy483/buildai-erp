from flask import Flask, request, redirect, url_for, session, render_template_string, render_template, flash, Response
...
def personal_panel():
    ...
    periods={"today":(total("income",today,today),total("expense",today,today)),"week":(total("income",week_start,week_end),total("expense",week_start,week_end)),"month":(total("income",month_start,today),total("expense",month_start,today))}
    cats=c.execute("SELECT category,SUM(amount) total FROM personal_transactions WHERE user_id=? AND transaction_type='expense' GROUP BY category ORDER BY total DESC",(uid,)).fetchall()
    ...
    return render_template("personal_panel.html",inc=inc,exp=exp,cats=cats,rows=rows,periods=periods)

def page(title, template, **ctx):
    body = render_template_string(template, **ctx)
    return render_template("base.html", title=title, body=body, personal=personal_panel(), version=VERSION)
